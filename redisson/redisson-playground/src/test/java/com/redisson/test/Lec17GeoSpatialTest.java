...
import com.redisson.test.dto.GeoLocation;
import com.redisson.test.dto.Restaurant;
import com.redisson.test.util.RestaurantUtil;
...
public class Lec17GeoSpatialTest extends BaseTest {
	...
	public void add() {
		Mono<Void> mono = Flux.fromIterable(RestaurantUtil.getRestaurants())
				...
				.flatMap(r -> this.map.fastPut(r.getZip(), GeoLocation.of(r.getLongitude(), r.getLatitude()))).then();
	...
	public void search() {
		Mono<Void> mono = this.map.get("75224")
				...
				.flatMap(r -> this.geo.search(r)).flatMapIterable(Function.identity()).doOnNext(System.out::println)
				.then();
