package com.redisspring.chat.service;
...
public class ChatRoomService implements WebSocketHandler {
	...
	public Mono<Void> handle(WebSocketSession webSocketSession) {
		...
		webSocketSession.receive().map(WebSocketMessage::getPayloadAsText)
				.flatMap(msg -> list.add(msg).then(topic.publish(msg))).doOnError(System.out::println)
				.doFinally(s -> System.out.println("Subscriber finally " + s)).subscribe();
	...
	private String getChatRoomName(WebSocketSession socketSession) {
		...
		return UriComponentsBuilder.fromUri(uri).build().getQueryParams().toSingleValueMap().getOrDefault("room",
				"default");
	}
